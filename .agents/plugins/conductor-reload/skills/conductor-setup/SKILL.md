...
| :--- | :--- | :--- |
| All files in `tracks/<track_id>/` (`spec`, `plan`, `metadata`, `index`) | **HALT** | "O projeto já está inicializado. Me diga qual feature você quer criar ou implementar." |
...
5.  **Inquire about Project Goal (for Greenfield):**
    ...
    -   **Upon receiving the user's response:**
        -   Execute `mkdir -p conductor/tracks`.
...
3.  **Action:** Construct and execute a command to create the directory and copy all selected files. For example: `mkdir -p conductor/code_styleguides && cp .agents/plugins/conductor-reload/skills/conductor-setup/templates/code_styleguides/python.md .agents/plugins/conductor-reload/skills/conductor-setup/templates/code_styleguides/javascript.md conductor/code_styleguides/`
4.  **Continue:** Immediately proceed to the next section.

### 2.5 Select Workflow (Interactive)
1.  **Copy Initial Workflow:**
    -   Copy `.agents/plugins/conductor-reload/skills/conductor-setup/templates/workflow.md` to `conductor/workflow.md`.
2.  **Determine Mode:** Use the `ask_user` tool to let the user choose their preferred workflow.
    - **questions:**
        - **header:** "Workflow"
...
3.  **Next Steps (Auto-Handoff Automático):** Anuncie que o setup está completo. Em seguida, **automaticamente**, leia e execute as instruções contidas em `.agents/plugins/conductor-reload/skills/conductor-review/SKILL.md` para a track recém-criada, iniciando a Revisão Constitucional sem exigir comando do usuário.
